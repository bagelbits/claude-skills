...
OUT=$(cd "$GITTMP" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs")
echo "$OUT" | grep -q "a.ts" && pass "violating branch denied naming file" || fail "violating branch not denied"

OUT=$(cd "$GITTMP" && echo '{"tool_name":"Bash","tool_input":{"command":"npm test"}}' | node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs")
[ -z "$OUT" ] && pass "unrelated Bash command skips the branch scan" || fail "unrelated Bash command still scanned the branch"

OUT=$(cd "$GITTMP" && echo '{"tool_name":"mcp__plugin_github_github__create_pull_request","tool_input":{}}' | node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs")
echo "$OUT" | grep -q "a.ts" && pass "gh MCP pull-request tool still scans the branch" || fail "gh MCP pull-request tool did not scan"

OUT=$(cd "$GITTMP" && node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs" < /dev/null)
...
NOTGIT=$(mktemp -d)
OUT=$(cd "$NOTGIT" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs")
...
OUT=$(cd "$GITTMP/sub" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-hat-check.mjs")
...
echo "== gate toggle: COMMENT_HAT_OFF =="
...
echo "$PAYLOAD" | CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" COMMENT_HAT_OFF=1 bash -c "$WRITE_CMD" > "$TOGGLE_TMP/on"
[ ! -s "$TOGGLE_TMP/on" ] && pass "COMMENT_HAT_OFF=1 bypasses write gate (no output)" \
  || fail "COMMENT_HAT_OFF=1 did not bypass write gate: $(cat "$TOGGLE_TMP/on")"

echo "$PAYLOAD" | CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" bash -c "$WRITE_CMD" > "$TOGGLE_TMP/off"
grep -q "x.ts" "$TOGGLE_TMP/off" && pass "write gate still denies with COMMENT_HAT_OFF unset" \
  || fail "write gate did not deny with COMMENT_HAT_OFF unset"
...
OUT=$(cd "$GATE_GITTMP" && echo "$COMMIT_PAYLOAD" | CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" COMMENT_HAT_OFF=1 bash -c "$COMMIT_CMD")
[ -z "$OUT" ] && pass "COMMENT_HAT_OFF=1 bypasses commit gate (no output)" \
  || fail "COMMENT_HAT_OFF=1 did not bypass commit gate: $OUT"

OUT=$(cd "$GATE_GITTMP" && echo "$COMMIT_PAYLOAD" | CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" bash -c "$COMMIT_CMD")
echo "$OUT" | grep -q "a.ts" && pass "commit gate still denies with COMMENT_HAT_OFF unset" \
  || fail "commit gate did not deny with COMMENT_HAT_OFF unset"
