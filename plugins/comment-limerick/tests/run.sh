...
OUT=$(cd "$GITTMP" && node "$PLUGIN_ROOT/hooks/pre-pr-limerick-check.mjs")
...
NOTGIT=$(mktemp -d)
OUT=$(cd "$NOTGIT" && node "$PLUGIN_ROOT/hooks/pre-pr-limerick-check.mjs")
...
OUT=$(cd "$GATE_GITTMP" && CLAUDE_PLUGIN_ROOT="$PLUGIN_ROOT" COMMENT_LIMERICK_OFF=1 bash -c "$COMMIT_CMD")
