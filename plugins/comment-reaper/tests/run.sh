...
OUT=$(cd "$GITTMP" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs")
echo "$OUT" | grep -q "a.ts" && pass "violating branch denied naming file" || fail "violating branch not denied"

OUT=$(cd "$GITTMP" && echo '{"tool_name":"Bash","tool_input":{"command":"npm test"}}' | node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs")
[ -z "$OUT" ] && pass "unrelated Bash command skips the branch scan" || fail "unrelated Bash command still scanned the branch"

OUT=$(cd "$GITTMP" && echo '{"tool_name":"mcp__plugin_github_github__create_pull_request","tool_input":{}}' | node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs")
echo "$OUT" | grep -q "a.ts" && pass "gh MCP pull-request tool still scans the branch" || fail "gh MCP pull-request tool did not scan"

OUT=$(cd "$GITTMP" && node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs" < /dev/null)
...
NOTGIT=$(mktemp -d)
OUT=$(cd "$NOTGIT" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs")
...
OUT=$(cd "$GITTMP/sub" && echo "$COMMIT_PAYLOAD" | node "$PLUGIN_ROOT/hooks/pre-pr-reaper-check.mjs")
