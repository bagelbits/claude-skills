...
claude plugin install comment-limerick@claude-skills
```
...
| [`comment-limerick`](plugins/comment-limerick) | Prose comments follow AABBA limerick form (five lines, real rhyme, anapestic meter); `//` line comments are denied on form. |

## Turning Off A Gate

Each poetry plugin (`comment-bard`, `comment-haijin`, `comment-in-the-hat`,
`comment-limerick`) can have its blocking write and commit/PR gate silenced
independently, without uninstalling the plugin — its rewrite skill stays
available either way. `comment-reaper` has no toggle; it's always on.
...
| `comment-limerick` | `COMMENT_LIMERICK_OFF=1` |
...
Install **at most one** of `comment-bard`, `comment-haijin`,
`comment-in-the-hat`, and `comment-limerick` — they enforce mutually
contradictory forms. None of the four pair with `comment-reaper`, which
wants comments *deleted* rather than versified.

## Shared engine

All five plugins are built on `packages/comment-core`, a shared,
